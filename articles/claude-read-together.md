...
1. **処理対象ファイルを確認する**
   - `zettel/1_literatures/Clippings/` 内のファイル一覧をGlobで取得する
...
3. **元記事をリネームする**
   ...
   - リネーム先ファイル：`zettel/1_literatures/Clippings/『{タイトル}』本文.md`
   - リネーム前にユーザーに確認は不要（タイトル確定済みのため）

4. **日本語翻訳ノートを作成する（英語記事のみ）**
   - 日本語記事の場合はこのステップをスキップする
   - ファイル：`zettel/1_literatures/Clippings/『{タイトル}』翻訳.md`
   - 元記事の本文全体を日本語に翻訳する
...
5. **要約ノートを作成する**
   - ファイル：`zettel/1_literatures/Clippings/『{タイトル}』要約.md`
...
6. **ファイルを移動する**
   - Bashで `mv` を使い、`zettel/1_literatures/Clippings/` から `zettel/1_literatures/` に移動する
...
---
name: read-together
description: 資料を一緒に読むスキル。clip-process で作成した翻訳・要約ノートや 1_literatures 内の資料ファイルを対象に「選択 → 読書 → 理解 → 確認テスト → 感想ノート → 読了」の学習セッションを Claude と一緒に実施する。「一緒に読もう」「この資料を読む」「read-together」「資料を読みたい」「クリップを読む」など、資料を読む作業を始めるときは必ずこのスキルを使う。
...
1. `zettel/1_literatures/` 以下の `.md` ファイルを Glob で取得する
...
4. ユーザーが資料を選択したら：
   ...
   - frontmatter に `status: reading` を設定する
     ...
     - frontmatter がない場合: ファイル先頭に以下を挿入する
       ...
       ---
       ```
5. clip-process 済みの要約.md を選択した場合は、翻訳ファイルへの案内を表示する：
...
**感想ノートのフォーマット**（`zettel/1_literatures/YYYY.M.D_『タイトル』感想.md`）：
...
3. ユーザーに案内する：「感想ノートをもとに、permanentノートを `zettel/2_permanents/` に作成してください。」
