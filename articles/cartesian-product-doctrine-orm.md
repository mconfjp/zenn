...
クエリ1は、`users`（親） - `posts`（子） - `comments`（孫）という関係性です。それに対してクエリ2は`posts`（子） - `users`（親） - `files`（子）という関係性です。`users`から`posts`と`files`の二つの子テーブルが生える形となっています。
これらの違いは図にするとわかりやすいです。

![](https://static.zenn.studio/user-upload/7e377531c645-20260703.png)
* わかりやすい図 *

このように、単純な親-子-孫という関係と、親に二つの子が結びつく関係では生まれる結果テーブルのレコード数が異なります。子親子というリレーションでは特定の親に紐づく子同士のデカルト積が結果セットの大きさに影響するため、これをデカルト積（直積）問題と呼んでいます。デカルト積問題の難点は、通常の親子孫リレーションが孫の数を見るだけで結果セットの大きさを予測できるのに対して、子親子のリレーションでは、特定の親にどれだけ紐づいたかという実データの分布が結果セットの大きさに影響してしまうことです。
...
### ライブラリ、環境
- PHP 8.3.31
- Doctrine ORM 3.6.7（DBAL 4.4.3）^[森田は普段Eloquentを使ってるんですが、今回の計測を]
### テーブル構造
以下のようなテーブル構造です。
```mermaid
erDiagram
...
| 6   | ピーク: 配列展開込み       | 69.12 MB                        | 417.17 MB                         |
| 7   | ピーク: ③のみ          | 156.54 MB                       | 102.36 MB                         |
...
## おわりに
以上で検証は完了です。
実測って楽しいですね！　皆さんもぜひいろんなデータを読み込んでは計測してみてください。なお環境のメモリ上限を超えてしまわないようにご注意を。
私はこの検証中に何度かトバしましたよ〜^[PHPのデフォルトメモリは128MBなので、余裕で飛びました]！
