...
#### ロックの前に考えること
ただ、ロックは並行性への対策として最初に手を出すものではありません。
先に検討できることが2つあります。

- **分離**：並行する利用者の間で、そもそもデータ領域を分けてしまう。同じレコードを複数人が触らない設計にできるなら、ロックの出番はありません
- **不変**：データが書き換わらないなら、何人が並行して読んでも問題は起きません。読み取り専用にできるデータは守る必要がありません

分離も不変も適用できず、それでも同じデータを複数のクライアントが書き換える。
ロックを考えるのはそこからです。

#### なぜDBのトランザクションでは足りないのか
ここで、DBのトランザクションを使えばいいのではないか、という疑問が出ます。
更新の喪失も一貫性のない読み込みも、教科書的にはトランザクション分離レベルで扱われる話だからです。

これができないのは、守りたい作業の単位とDBのトランザクションの単位がズレているからです。
トレーナーが編集画面を開いてから保存ボタンを押すまでには、数分かかることもあります。
その間ずっとDBのトランザクションを張り続けるわけにはいきません。
行ロックを数分間握ったままにすれば他の処理が詰まりますし、ユーザーが画面を閉じてしまえばトランザクションは宙に浮きます。

つまり、ユーザーから見た1つの作業（編集画面を開いて、直して、保存する）は、DBから見ると別々のトランザクションに分かれています。
前者をビジネストランザクション、後者をシステムトランザクションと呼びます[^poeaa]。
1つのビジネストランザクションが複数のシステムトランザクションに分かれるとき、その間を繋ぐ仕組みはアプリケーション側で用意するしかありません。
楽観ロックはそのための道具です。

[^poeaa]: 用語の定義はPoEAAの悲観的オフラインロックの項にあります。[Pessimistic Offline Lock](https://www.martinfowler.com/eaaCatalog/pessimisticOfflineLock.html)
